"""From https://github.com/BloodAxe/pytorch-toolbelt/blob/develop/pytorch_toolbelt/inference/tta.py # noqa
 Implementation of GPU-friendly test-time augmentation for image segmentation and
 classification tasks. Despite this is called test-time augmentation, these method can
 be used at training time as well since all transformation written in PyTorch and
 respect gradients flow.
"""
...
def fivecrop_image2label(model: nn.Module, image: Tensor, crop_size: Tuple) -> Tensor:
    """Test-time augmentation for image classification that takes five crops
    out of input tensor (4 on corners and central)
...
def tencrop_image2label(model: nn.Module, image: Tensor, crop_size: Tuple) -> Tensor:
    """Test-time augmentation for image classification that takes five crops out
    of input tensor (4 on corners and central) and averages predictions from them
    and from their horisontally-flipped versions (10-Crop TTA).
    :param model: Classification model
    :param image: Input image tensor
    :param crop_size: Crop size. Must be smaller than image size
    :return: Averaged logits
...
class MultiscaleTTAWrapper(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert (
            scale_levels or size_offsets
        ), "Either scale_levels or size_offsets must be set"
        assert not (
            scale_levels and size_offsets
        ), "Either scale_levels or size_offsets must be set"
        self.model = model
